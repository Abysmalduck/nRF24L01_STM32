...
HAL_StatusTypeDef nRF24::writeBuffer(uint8_t addr, uint8_t *buff, uint8_t size)
{
	...
	HAL_SPI_Transmit(_SPI_PORT, &addr, 1, HAL_MAX_DELAY);
	status =HAL_SPI_Receive(_SPI_PORT, buff, size, HAL_MAX_DELAY);

	HAL_GPIO_WritePin(_pin_csn->_gpio_type, _pin_csn->_gpio_num, GPIO_PIN_SET);
	return status;
}

HAL_StatusTypeDef nRF24::readeBuffer(uint8_t addr, uint8_t *buff, uint8_t size)
{
	HAL_StatusTypeDef status;
	addr = addr | NRF24_W_REGISTER;
	HAL_GPIO_WritePin(_pin_csn->_gpio_type, _pin_csn->_gpio_num, GPIO_PIN_RESET);
	...
	HAL_SPI_Transmit(_SPI_PORT, &addr, 1, HAL_MAX_DELAY);
	status = HAL_SPI_Transmit(_SPI_PORT, buff, size, HAL_MAX_DELAY);
