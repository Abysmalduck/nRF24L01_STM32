...
void nRF24::setup_Addr_length(uint8_t length)
{
	if ((length != NRF24_SETUPAW_3bytes) || (length != NRF24_SETUPAW_4bytes) || (length != NRF24_SETUPAW_5bytes))
	{
		writeReg(NRF24_REG_SETUP_AW, length);
	}
	return;
}
...
void nRF24::receive(uint8_t* data, uint8_t size)
{
	HAL_GPIO_WritePin(_pin_csn->_gpio_type, _pin_csn->_gpio_num, GPIO_PIN_RESET);
